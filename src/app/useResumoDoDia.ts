...
// quem terminar uma sessão (ou uma sincronização, de progresso ou de conteúdo) incrementa
// `versao` para forçar reler tudo de novo.
export function useResumoDoDia(alunoId: string, hoje: Date, versao: number): ResumoMateria[] | null {
  ...
  useEffect(() => {
    let cancelado = false
    Promise.all([indexarConteudo(), db.progresso.where('alunoId').equals(alunoId).toArray()]).then(([materias, progresso]) => {
      if (cancelado) return
