...
// quem terminar uma sessão incrementa `versao` para forçar reler o progresso salvo.
export function useResumoDoDia(hoje: Date, versao: number): ResumoMateria[] | null {
  ...
  useEffect(() => {
    let cancelado = false
    db.progresso.toArray().then((progresso) => {
      if (cancelado) return
